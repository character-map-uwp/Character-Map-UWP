...
public static class FlyoutHelper
{
        ...
        FlyoutArgs args)
    {
        ...
        MenuFlyoutItem Create(string key, ThemeIcons icon, RoutedEventHandler handler, VirtualKey accel = VirtualKey.None, bool add = true)
        {
        ...
        if (menu.Items != null)
        {
            ...
            MenuFlyoutSubItem coll;
            {
                ...
                if (!standalone)
                {
                    ...
                    if (showAdvanced is false && ResourceHelper.SupportsTabs)
                        Create("OpenInNewTab/Text", ThemeIcons.NewTab, OpenInNewTab);

                    // 1.2. Create "Open in New Window"
                    MenuFlyoutItem newWindow = Create("OpenInNewWindow/Text", ThemeIcons.NewWindow, OpenInNewWindow);
                    if (showAdvanced)
                ...
                {
                    Create("ExportFontFileLabel/Text", ThemeIcons.Save, SaveFont_Click, VirtualKey.S);
                    if (showAdvanced && !args.IsTabContext)
                        Create("ExportCharactersLabel/Text", ThemeIcons.Save, Export_Click, VirtualKey.E);
                }
            ...
            if (showAdvanced && args.IsTabContext is false)
            {
                ...
                {
                    MenuFlyoutItem print = Create("BtnPrint/Content", ThemeIcons.Print, Print_Click, VirtualKey.P, false);
            ...
            {
                menu.AddSeparator();
                MenuFlyoutItem del = Create("RemoveFontFlyout/Text", ThemeIcons.Delete, DeleteClick);
                if (showAdvanced)
            ...
            if (font.HasVariants)
                Create($"~{string.Format(Localization.Get("CompareFacesCountLabel/Text"), font.Variants.Count)}", ThemeIcons.CompareFonts, OpenFaceCompare);

            // 7.2. Add "Add to quick compare" button if we're viewing a variant
            if (qc)
                Create("AddToQuickCompare/Text", ThemeIcons.Add, AddToQuickCompare, VirtualKey.Q);

            // 8. Add Calligraphy button
            menu.AddSeparator();
            Create("CalligraphyLabel/Text", ThemeIcons.Calligraphy, OpenCalligraphy, VirtualKey.I);
        }
    }
    ...
    static FontIcon Icon(ThemeIcons icon)
    {
        return new FontIcon
        {
            Style = ResourceHelper.Get<Style>("ThemeFontIconStyle"),
            Glyph = ThemeIcon.Get(icon)
        };
    }
    ...
    public static MenuFlyoutSubItem AddCollectionItems(MenuFlyout menu, InstalledFont font, IReadOnlyList<InstalledFont> fonts, string key = null, FlyoutArgs args = null)
    {
        ...
        MenuFlyoutSubItem parent = new()
        {
            ...
            Icon = Icon(ThemeIcons.Collections),
            Style = substyle
        };
